...
public class UserRegisterServlet extends GenericServlet {
	...
	public void service(ServletRequest req, ServletResponse res) throws IOException, ServletException {
		...
		Pattern p=Pattern.compile("[3-9][0-9] {9}");
		Matcher m=p.matcher(phNo);
		boolean f=(m.find() && m.group().equals(phNo));
		//System.out.println(f);
		if(pWord.length()>=8 && (phNo).length()==10 && !f) {
		try {
		...
		else
		{
			RequestDispatcher rd = req.getRequestDispatcher("Sample.html");
			rd.include(req, res);
			pw.println("<h3 class='tab'>you have entered either Wrong Password or wrong Phone Number please Try Again.....!</h3>");	
		}
	}
